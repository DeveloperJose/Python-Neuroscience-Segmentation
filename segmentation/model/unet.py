...
class UpBlock(nn.Module):
    ...
    outchannels, via two convolution / relu pairs.
    """
    def __init__(self, inchannels, outchannels, dropout, spatial, kernel_size=3, stride=1):
        super().__init__()
        self.upconv = nn.ConvTranspose2d(
            inchannels, outchannels, kernel_size=2, stride=2
        )
        if dropout > 0:
            self.conv = ConvBlock(inchannels, outchannels, dropout, spatial, padding='same')
...
class Unet(nn.Module):
    ...
    def __init__(self, inchannels, outchannels, net_depth, dropout = 0.2, spatial = False, first_channel_output=16):
        ...
        for i in range(net_depth):
            k = 3 if i==0 else 3
            print(f'Down block with dropout {dropout[i]}')
            conv = ConvBlock(in_channels, out_channels, dropout[i], spatial, kernel_size=k, padding='same') #dropout[i]
            self.downblocks.append(conv)
            in_channels, out_channels = out_channels, 2 * out_channels
        
        print(f'Bottleneck block with dropout {dropout[-1]}')
        self.middle_conv = ConvBlock(in_channels, out_channels, dropout[-1], spatial, kernel_size=3) #dropout[-1]

        in_channels, out_channels = out_channels, int(out_channels / 2)
        for i in range(net_depth):
            print(f'Up block with dropout {dropout[:-1][::-1][i]}')
            upconv = UpBlock(in_channels, out_channels, dropout[:-1][::-1][i], spatial, kernel_size=3) #dropout[:-1][::-1][i]
